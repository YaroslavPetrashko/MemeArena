...
const tierGradient: Record<string, string> = {
  ...
  challenge: "from-red-700/50 to-zinc-900",
};

export function SnapBossArt({ boss, className }: { boss: SnapBoss; className?: string }) {
  const [loaded, setLoaded] = useState(false);
  const grad = tierGradient[boss.rewardTier] ?? tierGradient.low;
  return (
